 ...
 * @author maruhachi
 */
public class TickerF1 extends FXRateEvent {
    ...
    public TickerF1(OandaAutoTrader OAT, FXPair fxpair_ticker) {
    ...
    private void setLongbuy() {
        //SR[0]ヒストグラム短期 SR[1]=シグナル短期　SR[2]=MACD短期
        //SR[3]=ヒストグラム中期 SR[4]=シグナル中期 SR[5]=MACD中期
        //SR[6]=ヒストグラム長期 SR[7]=シグナル長期 SR[8]=MACD長期
    ...
    private void setLongRelease() {
        if ((currentAsk - currentBid) < 1) {
            ...
            boolean longSellFlag = ((SR[2] < SR[1]) && (SR[5] < SR[4]));
    ...
    private void setShortRelease() {
        if ((currentAsk - currentBid) < 1) {//スプレッドが1より下であれば取引する
            ...
            //SR[0]ヒストグラム SR[1]シグナル　SR[2]MACD
            //SR[3]=ヒストグラム長期 SR[4]=シグナル長期 SR[5]=MACD長期
            boolean shortSellFlag = ((SR[2] > SR[1]) && (SR[5] > SR[4]));//長短MACDがシグナルより上
            if (shortSellFlag) {
    ...
    private boolean orderCheck() {
        boolean check = false;
        try {
            check = setOrder.dealingCheck();
        } catch (AccountException ex) {
            Logger.getLogger(TickerF1.class.getName()).log(Level.SEVERE, null, ex);
        }
        if (!check) {
        ...
        //System.out.println("transactionnum:" + oat.transactionNum + " units:" + this.currentUnits);
        return check;
    }
    ...
    private boolean methodMacdHistogramFlag_SR0() {
    ...
    private boolean methodMacdHistogramFlag_SR3() {
